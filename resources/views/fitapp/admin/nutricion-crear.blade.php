...
<div class="admin-topbar">
    <div>
        ...
        <div class="admin-topbar-subtitle">Asigna comidas y alimentos reales a un cliente.</div>
    </div>
...
<form method="POST" action="{{ route('fitapp.admin.nutricion.store') }}" id="nutritionPlanForm">
    @csrf
    ...
    <div class="admin-grid-cards mb-4">
        <div class="admin-stat-card">
            ...
            <input type="number" step="0.01" name="target_calories" value="{{ old('target_calories') }}" class="form-control input-soft mt-2" placeholder="2100">
        </div>
        <div class="admin-stat-card">
            <div class="admin-stat-label"><i class="bi bi-lightning-charge me-1 text-primary-custom"></i>Proteina g</div>
            <input type="number" step="0.01" name="target_protein" value="{{ old('target_protein') }}" class="form-control input-soft mt-2" placeholder="160">
        </div>
        <div class="admin-stat-card">
            <div class="admin-stat-label"><i class="bi bi-bar-chart me-1 text-primary-custom"></i>Carbs g</div>
            <input type="number" step="0.01" name="target_carbohydrates" value="{{ old('target_carbohydrates') }}" class="form-control input-soft mt-2" placeholder="230">
        </div>
        <div class="admin-stat-card">
            <div class="admin-stat-label"><i class="bi bi-droplet-half me-1 text-primary-custom"></i>Grasas g</div>
            <input type="number" step="0.01" name="target_fat" value="{{ old('target_fat') }}" class="form-control input-soft mt-2" placeholder="60">
        </div>
    </div>
                ...
                <div class="admin-form-card-body">
                    <div class="row g-3">
                        <div class="col-md-5">
                            ...
                            <select class="form-select input-soft" name="user_id" required>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" @selected(old('user_id', $selectedUser?->id) == $user->id)>{{ $user->name }} - {{ $user->email }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold admin-label-icon"><i class="bi bi-card-heading"></i>Nombre</label>
                            <input type="text" name="name" value="{{ old('name', $selectedUser ? 'Plan de '.$selectedUser->name : '') }}" class="form-control input-soft" placeholder="Plan 2100 kcal" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label fw-bold admin-label-icon"><i class="bi bi-calendar-event"></i>Fecha</label>
                            <input type="date" name="plan_date" value="{{ old('plan_date', now()->toDateString()) }}" class="form-control input-soft">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold admin-label-icon"><i class="bi bi-bullseye"></i>Objetivo</label>
                            <input type="text" name="goal" value="{{ old('goal', $selectedUser?->goal) }}" class="form-control input-soft" placeholder="Masa muscular">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold admin-label-icon"><i class="bi bi-egg-fried"></i>Comidas por dia</label>
                            <input type="number" name="meals_per_day" value="{{ old('meals_per_day', $selectedUser?->meals_per_day ?: 5) }}" class="form-control input-soft" min="1" max="8" required>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-bold admin-label-icon"><i class="bi bi-droplet"></i>Agua diaria</label>
                            <input type="text" name="daily_water" value="{{ old('daily_water', '3 litros') }}" class="form-control input-soft" placeholder="3 litros">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="notes" class="form-control input-soft py-3" rows="3" placeholder="Indicaciones generales del plan.">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>

            @foreach ($mealNames as $mealIndex => $mealName)
                <div class="admin-form-card">
                    <div class="admin-form-card-head">
                                ...
                                <tbody>
                                    @for ($itemIndex = 0; $itemIndex < 4; $itemIndex++)
                                        <tr>
                                            ...
                                            <td><input type="number" step="0.01" name="meals[{{ $mealIndex }}][items][{{ $itemIndex }}][quantity]" class="form-control input-soft" placeholder="100"></td>
